...
class VNetEncoder(nn.Module):
    ...
    def __init__(self, elu=True, in_channels=1):
    ...
    def forward(self, x):
        results = []
        out16 = self.in_tr(x)
        results.append(out16)
        out32 = self.down_tr32(out16)
        results.append(out32)
        out64 = self.down_tr64(out32)
        results.append(out64)
        out128 = self.down_tr128(out64)
        results.append(out128)
        out256 = self.down_tr256(out128)
        return out256, results
...
class VNetDecoder(nn.Module):
    ...
    Implementations based on the Vnet paper: https://arxiv.org/abs/1606.04797
    """
    def __init__(self, elu=True, classes=1):
        super(VNetDecoder, self).__init__()
        self.classes = classes
    
        self.up_tr256 = UpTransition(512, 256, 2, elu, dropout=True)
        self.up_tr128 = UpTransition(256, 128, 2, elu, dropout=True)
        self.up_tr64 = UpTransition(128, 64, 1, elu)
        self.up_tr32 = UpTransition(64, 32, 1, elu)
        self.out_tr = OutputTransition(32, classes, elu)

    def forward(self, out256, features):

        out = self.up_tr256(out256, features[-1])
        out = self.up_tr128(out, features[-2])
        out = self.up_tr64(out, features[-3])
        out = self.up_tr32(out, features[-4])
        out = self.out_tr(out)
        return out
